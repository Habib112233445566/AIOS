# T-00840 — Regression Triage / CLI: Verification & Evidence

## 1. Verification Deliverables
- Fully validated CLI commands (`list`, `show`, `record`, `resolve`, `ingest`, `check`) in `aiosh-cli`.
- Automated test suites passing:
  - `tools/check_security_policy.py` (S1..S5 PASS).
  - `tools/check_task_docs.py` (C1..C6 PASS).
  - `tools/test_triage_suites.py` (T1..T3 PASS).
  - `tools/check_evidence.py` (E1..E4 PASS across 1931+ files).
...
[+] E1 directory-health: found 1931 evidence files
[+] E2 ledger-consistency: verified 50 sampled completed tasks
[+] E3 file-bounds: all 1931 files bounded and valid UTF-8
